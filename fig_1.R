...
quincy_low_copper <- quincy %>%
  ...
  geom_encircle(data = . %>% filter(Copper == 250),
                aes(group = as.factor(Copper), color = as.factor(Copper)), 
                size = 1.75, expand = 0.035, s_shape = .7, 
                show.legend = FALSE) +
  scale_color_manual(name = "Copper (µg/L)",
                     values = c("0" = "red",
                                "4" = "orange",
...
final_plot <- ggdraw(combined_plot) + 
  ...
  draw_label(expression(italic("Legionella") * " Relative Abundance"), x = 0.5, y = 0.02, size = 10, fontfamily = "Arial", fontface = "bold") +  # X-axis label
  draw_label(expression(italic("Neochlamydia") * " Relative Abundance"), x = .015, y = 0.5, size = 10, fontfamily = "Arial", angle = 90, fontface = "bold")
final_plot

ggsave("C:/Users/Jim Byrne/Documents/Phd/research/neochlamydia/letter_figs/Draft 14/fig_1.tiff",plot = last_plot(), width = 7, height = 4.167, dpi = 300, units = "in", grDevices::tiff)
...
uh <- flint %>%
  ...
  filter(Pipe == "PEX") %>%
  filter(Sampling == "Filter")
t.test(Legionella ~ Water, data = uh)

t.test(Neochlamydia ~ Water, data = uh)
